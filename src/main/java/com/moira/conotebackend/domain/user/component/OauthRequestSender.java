...
import static com.moira.conotebackend.global.constant.CoNoteConstant.*;
...
public class OauthRequestSender {
    ...
    private HttpEntity<MultiValueMap<String, String>> getKakaoTokenHttpEntity(String code) {
        // [1] Http Header
    ...
    private HttpEntity<MultiValueMap<String, String>> getNaverTokenHttpEntity(String code, String state) {
        // [1] Http Header
        HttpHeaders headers = new HttpHeaders();
        headers.add(HttpHeaders.CONTENT_TYPE, "application/x-www-form-urlencoded;charset=utf-8");
        ...
        body.add("code", code);
        body.add("state", "1234"); // TODO: 추후 수정 (임시값)
    ...
    public String getKakaoToken(String code) {
        // [1] Http 구조 생성
        HttpEntity<MultiValueMap<String, String>> request = getKakaoTokenHttpEntity(code);
    ...
    public String getNaverToken(String code, String state) {
        // [1] Http 구조 생성
        HttpEntity<MultiValueMap<String, String>> request = getNaverTokenHttpEntity(code, state);
        ...
        try {
            ...
            if (response.getBody() != null && response.getStatusCode().is2xxSuccessful()) {
                ...
                throw new CoNoteUserException(ErrorCode.NAVER_LOGIN_TOKEN_ERROR, HttpStatus.INTERNAL_SERVER_ERROR);
            }
        } catch (Exception e) {
            log.error("[네이버 로그인 오류] {}", ErrorCode.NAVER_LOGIN_ERROR.getMessage(), e);

            throw new CoNoteUserException(ErrorCode.NAVER_LOGIN_ERROR, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    public NaverUserInfoResponse getNaverUserInfo(String accessToken) {
        ...
        try {
            ...
            if (response.getBody() != null && response.getStatusCode().is2xxSuccessful()) {
                ...
                throw new CoNoteUserException(ErrorCode.NAVER_LOGIN_USER_INFO_ERROR, HttpStatus.INTERNAL_SERVER_ERROR);
            }
        } catch (Exception e) {
            log.error("[네이버 로그인 오류] {}", ErrorCode.NAVER_LOGIN_ERROR.getMessage(), e);
